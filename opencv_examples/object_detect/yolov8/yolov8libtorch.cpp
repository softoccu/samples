...
int main() {
    ...
    cv::VideoCapture stereo_cam(0); 
    if (!stereo_cam.isOpened()) {
    ...
    try {
        ...
        while (true) {
            ...
            left_frame = frame(cv::Rect(0, 0, frame.cols, frame.rows));
    
            cv::Mat input_image;
            letterbox(left_frame, input_image, {640, 640});
            ...
            keep.index_put_({Slice(), Slice(None, 4)}, scale_boxes({input_image.rows, input_image.cols}, boxes, {input_image.rows, input_image.cols}));
            ...
            for (int i = 0; i < keep.size(0); i++) {
                ...
                cv::rectangle(left_frame, cv::Point(x1, y1), cv::Point(x2, y2), cv::Scalar(0, 255, 0), 1);
                cv::putText(left_frame, classes[cls], cv::Point(x1, y1 - 10), cv::FONT_HERSHEY_SIMPLEX, 0.9, cv::Scalar(0, 255, 0), 1);
            }
            cv::imshow("Camera - Object Detection", left_frame);
